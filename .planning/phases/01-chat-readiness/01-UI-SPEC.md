...
| Schedule input prompts | Each setup value needing text input | Ask for a single value per message. Every time prompt must state WHICH value it is collecting before it states the format: a leading imperative sentence naming the value, taken from the wizard-sequence field name below, then the format sentence `Send a time in 24-hour format, for example 19:30.` That format sentence is fixed copy — prepend to it, never reword, split, or replace it. `Step X of 8` identifies progress, not content, and never substitutes for the subject sentence. Weekday and policy use inline keyboard choices. |
...
| Empty roster | `/roster` with no active members | Use the copywriting empty state below — its heading and its body, and no further instruction line — with no Remove controls. Copy for this surface lives ONLY in the Copywriting Contract, per the authority note under UI Considerations; this row states layout and behaviour and deliberately does not restate the sentence. |
