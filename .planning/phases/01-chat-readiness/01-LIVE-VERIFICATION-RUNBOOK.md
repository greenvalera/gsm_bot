...
| 3-03 | All eight wizard steps render per contract, including the distinct step 3/5/6 leading sentences and the untruncated `Previous participants` label | 2b | — | no | ❌ FAIL — the administrator walked all eight steps and Steps 2–8 matched their visual contract, including distinct Step 3/5/6 labels and the full `Previous participants` action. However, the operator rejected Step 1's instruction `Send a location in this group to choose this chat's time zone.` With Telegram privacy mode enabled, a group location reaches the bot only as a reply to the bot prompt; the copy omits that required gesture, makes the action hard to discover, and can make the bot appear frozen. The same sentence is used by setup and settings time-zone renderers. Recorded as F-12 / broken window 17. Screenshots were inspected but not retained, and location/time-zone and identity data were excluded. |
...
| 3-08 | A valid settings edit shows `Review change` with `Current:` / `New:` and saves in place | 3a | — | no | ✅ PASS — `Edit default start` prompted with the current value and 24-hour format, then a valid replacement produced bold `Review change`, `Current:`, `New:`, `Save change`, and `Keep current value`. Before save, committed revisions remained 1 and 5 while one active default-start edit held the replacement at expected revision 1. `Save change` replaced the review in place with the dashboard; afterward the edited configuration had revision 2 and zero settings-edit drafts remained. Screenshots were inspected but not retained, and the selected zone and identities were excluded. |
| 3-09 | A conflicting schedule returns `That schedule does not fit inside the daily time boundaries. No changes were saved.` and mutates nothing | 3b | — | no | ✅ PASS — from revision 2, `Edit default start` received a value whose rehearsal duration would extend past the saved daily end. The bot replied exactly `That schedule does not fit inside the daily time boundaries. No changes were saved.` A sanitized read immediately afterward still found committed revisions 2 and 5; the one active default-start edit expected revision 2 and had no replacement payload. The conflicting value therefore changed neither committed configuration nor draft replacement. The screenshot was inspected but not retained, and its zone and identities were excluded. |
| 3-10 | `Edit daily start` and `Edit daily end` are separate, UI-reachable rows, and an edit **completes end-to-end through `Edit daily end`** — the half Run 2 left as existence-only | 3c | — | no | ✅ PASS — the post-restart dashboard displayed separate `Edit daily start` and `Edit daily end` actions. The administrator opened `Edit daily end`, entered a valid later boundary, and received `Review change` with Current/New and both save/keep actions. Before save, one active `DAILY_END_MINUTE` replacement expected revision 2 while committed revisions stayed 2 and 5. `Save change` replaced the review with the dashboard showing the updated daily end; afterward the edited configuration was revision 3 and zero settings-edit drafts remained. Screenshots were inspected but not retained, and zone/identity data were excluded. |
| 3-11 | `/roster_add` as a reply adds the member immediately, with no confirmation click | 4a | — | no | ✅ PASS — a normal member sent a group message and the administrator replied directly to it with `/roster_add`. The bot immediately returned the check-mark `Added <member> to the band roster.` success sentence with no intermediate confirmation action. A sanitized baseline afterward found two membership rows across two chats: one active and one historical inactive row, plus two Telegram-user rows. The screenshot was inspected but not retained; member/admin names, usernames, avatars, message text, group identity, IDs, and zone were excluded. |
| 3-12 | A repeat `/roster_add` returns `✅ <member> is already in the band roster.` and inserts no duplicate row | 4b | — | no | ✅ PASS — the administrator repeated `/roster_add` as a reply to the same member message. The bot returned exactly `✅ <member> is already in the band roster.` Anonymous counts stayed identical to the post-add baseline: two membership rows across two chats, one active and one historical inactive, plus two Telegram-user rows. No duplicate membership or user row was inserted. The screenshot was inspected but not retained, and all identities, message text, IDs, group details, and zone were excluded. |
| 3-13 | `/roster` renders safely; the full numeric ID is nowhere visible | 4c | — | no | ✅ PASS — `/roster` rendered bold `Band roster`, one bullet entry in the permitted name-only form, and exactly one `Remove member` button. No full or partial numeric Telegram ID, raw callback target, or internal identifier was visible. The screenshot was inspected but not retained; the displayed member/admin identities, message text, group details, and zone were excluded. |
| 3-14 | Removal dialog verbatim copy, and `Keep member` leaves the member active | 4d | — | no | ✅ PASS — `Remove member` replaced the roster card with bold `Remove <member>?`, body `They will no longer be selected for future rehearsals.`, and buttons `Remove member` / `Keep member`. Choosing `Keep member` replaced that dialog with `Removal cancelled.` Anonymous membership counts stayed at two total, one active and one historical inactive, proving the member remained active. Screenshots were inspected but not retained, and all visible identity/group details were excluded. |
| 3-15 | Confirmed removal replaces the card together with its buttons; soft delete in the database | 4e | — | no | ✅ PASS — after reopening `/roster`, the administrator selected `Remove member` and confirmed `Remove member` in the dialog. The dialog and keyboard were replaced together by bold `Roster updated` plus `They will no longer be selected for future rehearsals.` with no buttons remaining. Anonymous data then showed two historical membership rows, zero active, two inactive, and both inactive rows carrying deactivation timestamps: the current membership was soft-deleted without removing history. The screenshot was inspected but not retained, and identities/group details were excluded. |
| 3-16 | Restart with a populated roster: both configuration and roster match the saved projection | 4f | — | no | ✅ PASS — after confirmed removal, the same historical membership was reactivated without inserting a row: totals returned to two membership rows, one active and one inactive. Root restarted only the bot and verified changed bot start time, preserved/healthy PostgreSQL, preserved named volume, and one fresh long-poll startup record. Post-restart `/settings` showed the saved revision-3 configuration and `/roster` showed the active member. Data still held configuration revisions 3 and 5, one active membership, and no active setup/settings draft. The screenshot was inspected but not retained; configuration values, zone, identities, and IDs were excluded. |
| 3-17 | Demotion: command denial, private callback alert `Only current chat administrators can do that.`, draft deleted **before** the denial | 5c | — | no | ✅ PASS — the second account, while temporarily an administrator, opened `Edit weekday`; sanitized data showed one active `DEFAULT_WEEKDAY` draft with no replacement at expected revision 3. After the owner removed that account's administrator rights, tapping the still-live weekday keyboard produced the private modal alert `Only current chat administrators can do that.` An immediate read found zero settings drafts while committed revisions stayed 3 and 5. The still-demoted account then sent `/settings` and received exactly `Only current chat administrators can change chat setup, roster, or planning access.` in the group; a repeat read still showed no draft or mutation. Screenshots were inspected but not retained, and identities, group details, IDs, configuration values, and zone were excluded. |
| 3-18 | After rights are restored the previous draft is **not** resumed | 5d | — | no | ✅ PASS — after rights were restored, the second account could open `/settings` again, then `/setup` produced exactly one `Setup in progress` response at `Step 1 of 8` asking for a location. It did not resume the deleted weekday edit. A sanitized read found one active, completely blank setup draft at expected revision 3, zero active settings drafts, and unchanged committed revisions 3 and 5. The screenshot was inspected but not retained, and identities, group details, configuration values, IDs, and zone were excluded. |
| 3-19 | An ordinary message from a demoted actor produces complete silence in the chat and `"outcome":"no-in-flight-action"` in the log | 5e | — | no | ✅ PASS — while still demoted and with no draft, the second account replied ordinary text directly to the bot's latest denial. After waiting, the captured chat showed no bot response. From a UTC boundary recorded before the reply, sanitized log counts found exactly one `"outcome":"no-in-flight-action"`, zero `denied` outcomes, and zero fatal/error records. The screenshot was inspected but not retained; reply text, identities, group details, IDs, and configuration data were excluded. |
| 3-20 | Empty roster shows exactly `No band members yet` + `Reply to a member's message, then send /roster_add to add them.`, no Remove buttons | 6a | — | no | ✅ PASS — after the owner confirmed removal of the sole active member, the removal dialog was replaced with `Roster updated`; a fresh `/roster` then rendered exactly bold `No band members yet` and `Reply to a member's message, then send /roster_add to add them.` with no third line and no Remove button. Anonymous data showed two historical membership rows, zero active, two inactive, and both soft-deleted. The screenshot was inspected but not retained, and identities/group details were excluded. |
...
| 3-22 | Stale actions: the setup alert and the settings/roster alert are worded differently and each reaches its own branch | 6c | — | no | ✅ PASS — the owner tapped an old unconfigured-chat `Start setup` action and received the private alert `This setup action is no longer available. Send /setup to start again.` The owner then tapped an old settings `Edit …` action and received the distinct private alert `This action is no longer available. Open /settings or /roster and try again.` Sanitized data afterward still showed committed revisions 3 and 5, zero active settings drafts, and the other actor's one active setup draft unchanged. Screenshots were inspected but not retained; old map/configuration values, identities, IDs, group details, and zone were excluded. |
| 3-23 | Client nativeness: native wrapping, no reply keyboard, no WebView, cards replaced rather than duplicated | 6d | — | no | ✅ PASS — direct Telegram screenshots across setup, settings, roster, denial, and stale-action flows showed native text wrapping, inline keyboards only, no reply keyboard, and no WebView. Callback transitions replaced their originating cards: candidate → Step 2, reminders → Step 8, Step 8 → review, review/save → dashboard, settings review/save → dashboard, and roster confirmation/cancel/removal → terminal state without leftover buttons. `Previous participants` rendered fully and each Step 8 policy occupied its own row. Screenshots were visually inspected only and not retained. |
