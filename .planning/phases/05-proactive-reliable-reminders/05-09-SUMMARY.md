...
key-files:
  ...
  modified: [src/domain/chat/migration-service.ts, src/domain/reminders/reminder-service.ts, src/app/main.ts, src/infrastructure/jobs/reminder-queue.ts, src/app/create-bot.ts, src/shared/chat-coordinator.ts]
key-decisions:
...
- Persistent signal listeners remain installed during the entire drain, preventing a repeated SIGTERM or SIGINT from restoring Node's default immediate termination. The coordinator separately tracks actual update middleware and cuts off both new and queued update admission; it does not trust runner.stop as proof that middleware completed.
...
**3. [Rule 1 - Signal and runner lifecycle] Track actual middleware rather than runner.stop.**
...
- **Shutdown limitation:** arbitrary already-running update middleware cannot be canceled safely. Its 30-second drain deadline records failure, but database disconnection waits for that middleware to settle, which can exceed the deadline indefinitely. This deliberately protects accepted transactional work from late database reconnects. Reminder HTTP draining remains strictly bounded and ignores late transport results. Thus the whole-process shutdown duration is not guaranteed bounded when an accepted update handler never settles; independent review/phase verification must retain this distinction.
...
- All eight listed source/test deliverables exist, and all task/review commit hashes exist.
...
- Actuals are `ceil(realized diff characters / 4)` for the eight files through the final update-drain source commit, not harness token usage.
