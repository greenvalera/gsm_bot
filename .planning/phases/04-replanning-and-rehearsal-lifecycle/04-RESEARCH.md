...
| A6 | ~~Telegram imposes no practical age limit on a bot editing its own group message.~~ **DISCHARGED — no longer an assumption.** Confirmed against the Bot API during planning: `editMessageText`'s only time bound is scoped to business messages not sent by the bot and lacking an inline keyboard, none of which describes this phase's edits. See Open Question 3 for the citation and the `deleteMessage` contrast. | Open Question 3 (RESOLVED) | No residual risk. The mitigations that were carried while this was open (D-15's new-message primacy, the Phase 3 D-03 no-rollback rule, `PLANNING_CATCH_SITES.delivery` absorption) are retained regardless, because a neuter can still fail for unrelated reasons. |
...
> All four questions below are closed. Q1 → resolved as a decision record in `04-04-PLAN.md`. Q2 →
> resolved in `04-01-PLAN.md` Task 2 §5 and assumption A1. Q3 → resolved here, during planning, by the
> live Bot API check the question itself prescribed. Q4 → answered below and given a test in
> `04-02-PLAN.md` Task 1 §6.
...
3. **Is there an age limit on editing a bot's own group message?** — **RESOLVED: no. There is no age limit on this phase's edits.**
   - **How it was resolved.** The two `WebFetch` attempts during research truncated before the method sections. During planning the page was retrieved in full instead (`curl https://core.telegram.org/bots/api`, 860 KB) and the sections were read locally, which is the live check this question asked for.
   - **Finding — `editMessageText`.** The method's only time-bounded sentence is: *"Note that business messages that were not sent by the bot and do not contain an inline keyboard can only be edited within 48 hours from the time they were sent."* [CITED: https://core.telegram.org/bots/api#editmessagetext]. **Every one of its three conditions fails for this phase**, independently: the messages are *sent by the bot*; they are *group* messages, not business-connection messages; and every one this phase edits *carries an inline keyboard*. No unconditional age limit is stated for `editMessageText`, and none appears anywhere else on the page. The identical sentence, with the identical scope, is the only time bound on `editMessageCaption`, `editMessageMedia` and `editMessageReplyMarkup` [CITED: https://core.telegram.org/bots/api#editmessagereplymarkup].
   - **Contrast — `deleteMessage` DOES carry a hard limit:** *"A message can only be deleted if it was sent less than 48 hours ago."* [CITED: https://core.telegram.org/bots/api#deletemessage]. This is the constraint the question was reaching for, and it lands on a method this phase deliberately does not use — `COVERAGE.md` records `deleteMessage` as an explicit OPT-OUT, because D-08 and D-15 neuter a superseded or cancelled message *in place* so the chat keeps a readable trail. **The one Bot API age limit in this area is therefore designed out of the phase rather than depended on.**
   - **Consequence for D-10.** `/plan_cancel` may edit the announcement of a rehearsal booked arbitrarily long ago. Assumption A6 is discharged as verified fact, not carried as risk.
   - **Defence in depth retained anyway.** Nothing is relaxed on the strength of this finding: D-15 keeps the new group message as the *primary* notification of a booked cancellation, the Phase 3 D-03 rule still prevents a failed edit from rolling back a committed transition, and `PLANNING_CATCH_SITES.delivery` still absorbs and logs a failed edit. A neuter that fails for any *other* reason (message manually deleted, bot removed from the chat, network) degrades to the same safe outcome.
...
- [Telegram Bot API — `editMessageText`](https://core.telegram.org/bots/api#editmessagetext), [`editMessageReplyMarkup`](https://core.telegram.org/bots/api#editmessagereplymarkup), [`deleteMessage`](https://core.telegram.org/bots/api#deletemessage) — **retrieved during planning**, after two research-phase `WebFetch` attempts truncated. The full page was fetched with `curl` (860 KB) and the method sections read locally, which is why the age-limit question could finally be answered from the source rather than assumed. Quoted verbatim in Open Question 3. Same provider caveat: `[CITED]`, not `[VERIFIED]`.

### Tertiary (LOW confidence)

- None. The one entry formerly recorded here — the `editMessageText` / `deleteMessage` age constraints — was retrieved during planning and promoted to Secondary above; assumption A6 and Open Question 3 are both closed.
