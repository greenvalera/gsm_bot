...
describe("protected settings authorization", () => {
  it("permits creators and administrators, and deletes only the denied actor drafts", async () => {
    ...
    const prisma = {
      setupDraft: { async deleteMany({ where }: any) { deleted.push(where); } },
      settingsEditDraft: {
    ...
    await expect(allowed.requireCurrentAdministrator(1n, 2n)).resolves.toBeUndefined();
    ...
    await expect(denied.requireCurrentAdministrator(1n, 2n)).rejects.toBeInstanceOf(
      PermissionDeniedError,
    );
    expect(deleted).toEqual([{ chatId: 1n, actorUserId: 2n }, { chatId: 1n, actorUserId: 2n }]);
  });
});
