...
describe.each(["en", "uk"] as const)("lifecycle cards in %s", (locale) => {
  it("distinguishes readiness, blocked and retracted facts with safe names", () => {
    ...
    const blocked = renderBlockedAnnouncement(
      projection,
      noTokens,
      locale,
    ).text;
    ...
    expect(blocked).toContain(
      locale === "uk"
        ? "Не можуть прийти: Оля &lt;&amp;&gt;."
        : "Cannot attend: Оля &lt;&amp;&gt;.",
    );
    expect(retracted).toContain(
      locale === "uk"
        ? "Попереднє оголошення вже не актуальне"
        : "The earlier announcement no longer stands",
    );
  ...
  it("asks about an external booking and records it without attribution", () => {
    ...
    const booked = renderBookingConfirmation(
      { ...projection, booked: true },
      noTokens,
      locale,
    );
    expect(booked.text).toContain(
  ...
  it("keeps cancellation, successor and retired-copy recovery distinct", () => {
    ...
    expect(
      renderCancellationNotice(round, projection.participants, locale).text,
    ).toContain(
      locale === "uk"
        ? "Цю репетицію скасовано."
        : "This rehearsal was cancelled.",
    );
    expect(renderSupersededAttemptLine(round, locale).text).toContain(
    ...
    expect(retired).toContain(
      locale === "uk"
        ? "Ця копія вже не актуальна."
        : "This copy is no longer current.",
    );
  ...
  it("localizes incomplete slots and masked identities", () => {
    ...
    expect(
      renderCancellationConfirmation(empty, noTokens, locale).text,
    ).toContain(locale === "uk" ? "план репетиції" : "the rehearsal plan");
    expect(renderSupersededAttemptLine(empty, locale).text).toContain(
      locale === "uk"
        ? "Попередню спробу планування"
        : "The previous planning attempt",
    );
    const unknown = { ...projection.participants[0]!, firstName: null };
    expect(renderCancellationNotice(round, [unknown], locale).text).toContain(
      locale === "uk"
