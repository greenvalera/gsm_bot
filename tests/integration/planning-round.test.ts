...
describe("planning round vertical slice", () => {
  it("starts a durable week-aware round and anchors the day card for a non-admin author", async () => {
    ...
    expect(keyboardButtons(sent)).toHaveLength(9);
    expect(keyboardRows(sent).map((row) => row.length)).toEqual([4, 3, 1, 1]);
  ...
  it("replaces the same anchor card in place when the author taps a day", async () => {
    ...
    expect(keyboardRows(edited).at(-3)).toEqual([PLANNING_BACK_LABEL]);
...
describe("a booked round carries the same weight as a confirmed one (D-15)", () => {
  ...
  it("uses the scheduled-end boundary and excludes cancelled or superseded attempts", async () => {
    const chatId = -1007000000091n;
    await configureChat(chatId);
    const service = new PlanningService(prisma);
    const row = await seedFinishedRound(chatId, "2026-08-24", "CONFIRMED", {
    ...
    for (const status of [
      ...
      expect((await service.previousRehearsal(chatId, NOW))?.id ?? null).toBe(
        status === "CONFIRMED" || status === "BOOKED" ? row.id : null,
      );
    }
  });
  it("orders finished rehearsals by start even when the earlier one ends later", async () => {
    const chatId = -1007000000092n;
    await configureChat(chatId);
    await seedFinishedRound(chatId, "2026-08-17", "BOOKED", {
      startsAt: new Date(NOW.getTime() - 14400000),
      endsAt: new Date(NOW.getTime() - 1000),
    });
    const later = await seedFinishedRound(chatId, "2026-08-17", "CONFIRMED", {
