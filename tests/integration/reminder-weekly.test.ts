...
describe("weekly reconciliation", () => {
  ...
  const service = () => new ReminderService({prisma, now: () => at, logger: createLogger({level: "silent"}), transport});
  ...
  async function round(status: "DRAFT" | "CONFIRMED" | "BOOKED", week = "2026-09-14") {
    return prisma.planningRound.create({data: {chatId, authorUserId: 1n, targetWeekStart: week, activeWeekStart: status === "DRAFT" ? week : null, status, timezone: "Europe/Kyiv", durationMinutes: 120, dailyStartMinute: 600, dailyEndMinute: 1260, lastActivityAt: at}});
  }
  ...
  it.each(["DRAFT", "CONFIRMED", "BOOKED"] as const)("suppresses %s current week, does not chase next week before Monday", async status => {
    await round(status);
    await service().reconcile();
    ...
    await service().reconcile();
    expect(transport).toHaveBeenCalledWith(expect.objectContaining({targetWeek: "2026-09-21"}));
  });
  it("setup exactly at 10:00 waits for tomorrow", async () => {
    await prisma.chatReminderState.update({where:{chatId},data:{effectiveFrom: at}});
    await service().reconcile();
    expect(transport).not.toHaveBeenCalled();
  ...
  it("rechecks draft after generation, independently from week claim", async () => {
    ...
    await service().reconcile();
    expect(transport).not.toHaveBeenCalled();
    expect(await prisma.reminderOccurrence.count({where:{disposition:"OBSOLETE"}})).toBe(1);
  });
  ...
  it("ignores future manually planned weeks and quiet state is authoritative", async () => {
    ...
    await service().reconcile();
    expect(transport).toHaveBeenCalledTimes(1);
    await prisma.chatReminderState.update({where:{chatId},data:{quietUntil:new Date("2026-09-21T00:00Z")}});
    at = new Date("2026-09-17T07:00Z");
    await service().reconcile();
    expect(transport).toHaveBeenCalledTimes(1);
  });
