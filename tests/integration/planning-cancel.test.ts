...
import { weekIsClaimed } from "../../src/domain/planning/target-week.js";
...
async function offer(
  ...
  return {
    result,
    apply: result.actions.find((a) => a.target.action === "cancel-apply")!
      .token,
    keep: result.actions.find((a) => a.target.action === "cancel-keep")!.token,
  };
}
...
describe("cancellation transactions and standing", () => {
  ...
  it.each(["DRAFT", "CONFIRMED", "BOOKED"] as const)(
    ...
    async (status) => {
      ...
      expect(result.round).toMatchObject({
        status: "CANCELLED",
        cancelledAt: NOW,
        cancelledByUserId: AUTHOR,
        activeWeekStart: null,
        revision: f.round.revision + 1,
      });
      expect(
      ...
      expect((await service.status(f.chatId, MEMBER, NOW)).kind).toBe(
        "no-active-round",
      );
      expect(
        (
  ...
  it("rechecks demotion, preserves refused tokens, and admits a fresh administrator", async () => {
    const f = await fixture();
    const o = await offer(f, ADMIN, "administrator");
    expect(
      (
        await service.applyCancel(
          f.chatId,
  ...
  it("expires prior pairs, declines safely, releases a stale consume, and refuses superseded attempts", async () => {
    ...
    expect(
      (
        await prisma.callbackAction.findUniqueOrThrow({
          where: { token: second.apply },
        })
      ).consumedAt,
    ).toBeNull();
    expect(
      (
        await service.keepCancel(
          f.chatId,
          AUTHOR,
          second.keep,
          NOW,
