...
describe("shared worker/update coordination", () => {
  it.each(["settings", "cancel", "response"])(
    "holds %s update behind in-flight worker and permits another chat",
    async (kind) => {
      const coordinator = new ChatCoordinator();
      const entered = latch();
      const finish = latch();
      const order: string[] = [];
      const worker = coordinator.run(["chat:-1"], async () => {
        entered.release();
        await finish.promise;
        order.push("send");
      });
      await entered.promise;
      const update = coordinator.run(["chat:-1"], async () => {
        order.push(kind);
      });
      await coordinator.run(["chat:-2"], async () => {
        order.push("other");
      });
      expect(order).toEqual(["other"]);
      finish.release();
      await Promise.all([worker, update]);
  ...
  it("migration obtains old/new keys once in sorted order without deadlock", async () => {
    const coordinator = new ChatCoordinator();
    const fake = {
      message: { migrate_to_chat_id: -1002, chat: { id: -1, type: "group" } },
    } as Context;
    ...
    const migration = coordinator.run(
      [...migrationKeys(fake), "chat:-1"].reverse(),
      async () => {
        entered.release();
        await hold.promise;
    ...
    await entered.promise;
    const old = coordinator.run(["chat:-1"], async () => {
      events.push("old");
    });
    const next = coordinator.run(["chat:-1002"], async () => {
      events.push("new");
    });
    expect(events).toEqual([]);
    hold.release();
    await Promise.all([migration, old, next]);
