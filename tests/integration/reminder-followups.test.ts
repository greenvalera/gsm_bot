...
const chatId = -708n,
  due = new Date("2026-09-16T07:00Z");
beforeAll(async () => {
...
async function fixture() {
  const round = await prisma.planningRound.create({
    data: {
      ...
      firstAvailabilityPublishedAt: new Date("2026-09-16T06:00Z"),
      availabilityAnchorAcknowledgedAt: new Date("2026-09-16T06:00Z"),
    },
  });
  for (const id of [3n, 2n, 1n, 4n]) {
    await prisma.telegramUser.upsert({
      ...
      create: {
        telegramUserId: id,
        firstName: String.fromCharCode(64 + Number(id)),
      },
      update: { firstName: String.fromCharCode(64 + Number(id)) },
    });
    const membership = await prisma.chatMembership.create({
      data: { chatId, telegramUserId: id, activeAt: id === 1n ? null : due },
    });
    if (id !== 4n)
      await prisma.planningParticipant.create({
        data: {
          ...
          availability: id === 2n ? "AVAILABLE" : null,
        },
      });
  }
  return round;
...
async function occurrence(roundId: string, at = due) {
  return prisma.reminderOccurrence.create({
    data: {
      ...
      minute: at.getUTCHours() * 60 + at.getUTCMinutes() + 180,
      dueAt: at,
    },
  });
}
